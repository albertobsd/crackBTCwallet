...
const char *version = "0.1.20211119";
      ...
      case 1:
        ...
        switch(indexOf(token,commands1,8))  {
          ...
          if(STATUS == 0)  {
			...
			*/
			if(RANDOMSOURCE > 0)	{
				devurandom = fopen("/dev/srandom","rb");
				printf("opening /dev/srandom as random source\n");
			}
			else {
        ...
        case 3:  //stats
			...
			//printf("Total AES Operations %lu @ %is (%.0f)\n",total,seconds,(double) ((uint64_t)total/seconds));
